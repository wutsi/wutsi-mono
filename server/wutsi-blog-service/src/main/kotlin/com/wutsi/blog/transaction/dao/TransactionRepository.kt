...
interface TransactionRepository : CrudRepository<TransactionEntity, String> {
    ...
    @Query("SELECT SUM(T.amount) FROM TransactionEntity T WHERE T.campaign=?1 AND T.status=?2")
    fun sumAmountByCampaignAndStatus(campaign: String, status: Status): Long?
