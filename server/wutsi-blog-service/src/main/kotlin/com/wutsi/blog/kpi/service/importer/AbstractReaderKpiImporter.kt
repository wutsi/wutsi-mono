...
abstract class AbstractReaderKpiImporter(
    ...
    override fun import(date: LocalDate, file: File): Long {
        val kpis = loadKpis(file, KpiReaderFilterNone())
        import(kpis, date)
        return kpis.size.toLong()
    }

    protected fun loadKpis(file: File, filter: KpiReaderFilter): List<KpiReader> {
...
interface KpiReaderFilter {
    fun accept(record: CSVRecord): Boolean
}

class KpiReaderFilterNone : KpiReaderFilter {
