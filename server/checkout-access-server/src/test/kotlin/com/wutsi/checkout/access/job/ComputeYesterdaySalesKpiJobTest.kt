...
internal class ComputeYesterdaySalesKpiJobTest {
    @Autowired
    private lateinit var job: ComputeYesterdaySalesKpiJob
    ...
    fun run() {
        ...
        val path = "kpi/" + yesterday.format(DateTimeFormatter.ofPattern("yyyy/MM/dd")) + "/views.csv"
        ...
        // THEN
        assertKpi(3, 6, 9000, 10, 1, 100, yesterday)
        assertKpi(1, 1, 500, 11, 1, 101, yesterday)
        assertKpi(1, 1, 1500, 20, 2, 200, yesterday)
        assertKpi(0, 0, 0, 50, 2, 201, yesterday)
        ...
        val input =
            FileInputStream(File("$storageDirectory/kpi/" + yesterday.format(DateTimeFormatter.ofPattern("yyyy/MM/dd")) + "/sales.csv"))
        input.use {
            assertEquals(
                """
