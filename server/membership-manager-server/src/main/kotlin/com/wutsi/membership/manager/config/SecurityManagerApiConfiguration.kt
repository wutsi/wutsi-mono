...
import com.wutsi.security.manager.SecurityManagerApiBuilder
...
@Configuration
class SecurityManagerApiConfiguration(
    ...
    companion object {
        private val LOGGER = LoggerFactory.getLogger(SecurityManagerApiConfiguration::class.java)
    }
    ...
    @Bean
    fun securityManagerApi(): SecurityManagerApi =
        SecurityManagerApiBuilder().build(
            env = environment(),
            mapper = mapper,
            interceptors = listOf(
    ...
    private fun environment(): com.wutsi.security.manager.Environment =
        if (env.acceptsProfiles(Profiles.of("prod"))) {
            com.wutsi.security.manager.Environment.PRODUCTION
        } else {
            com.wutsi.security.manager.Environment.SANDBOX
        }
}
